...
def add_to_cart(request, productid, quantity):
    if request.method == 'POST':
        current_user = request.user.id
        if CartTest.objects.filter(user_id__exact=current_user, product_id=productid):
            existing_cart = CartTest.objects.get(product_id=productid, user_id=current_user)
            print(existing_cart.quantity)
            existing_cart.quantity = existing_cart.quantity + 1
            print(existing_cart.quantity)
            existing_cart.save()
